...
from gi.repository import Gtk, GObject, Gdk, GdkPixbuf, GtkSource
import pygame
...
class Activity(sugar3.activity.activity.Activity):
    def __init__(self, handle):
        ...
        eb = Gtk.EventBox()
        eb.add(self.splash)
        eb.modify_bg(Gtk.StateFlags.NORMAL, Gdk.color_parse("white"))
        eb.show()
        self.box.append_page(eb, Gtk.Label("Inicio"))

        self.game = game.colgadito_gui.Game
        self.game.canvas = self._pygamecanvas = \
            sugargame.canvas.PygameCanvas(self,
                main=game.colgadito_gui.Game,
    ...
    def save_file(self, widget):
        if self.editor.modificado():
            ...
            GOject.timeout_add(1500, self.check_modified)
    ...
    def restart_game(self, widget):
        ...
        self.show_game(None)
        watch = Gdk.Cursor(Gdk.CursorType.WATCH)
        self.get_plug_window().set_cursor(watch)
        JUEGO = reload(JUEGO)
        self.game = JUEGO.Game(activity=self)
